...
[`MILESTONE/test-outputs/README.md`](./test-outputs/README.md).

| # | Gate | Captured log | Sentinel observed |
|---|---|---|---|
| 1 | `npm run type-check:trust-core` | [`01-type-check-trust-core.txt`](./test-outputs/01-type-check-trust-core.txt) | exit 0; no `error TS` |
| 2 | `npm run type-check:x402-client` + `build:x402-client` | [`02-type-check-x402-client.txt`](./test-outputs/02-type-check-x402-client.txt) | exit 0 (both passes); CJS dist emitted |
| 3 | `npm run type-check:trust-api` | [`03-type-check-trust-api.txt`](./test-outputs/03-type-check-trust-api.txt) | exit 0; no `error TS` |
| 4 | `npm run type-check:mcp-server` + `build:mcp-server` | [`04-type-check-mcp-server.txt`](./test-outputs/04-type-check-mcp-server.txt) | exit 0 (both passes); dist emitted |
| 5 | `npm --workspace @arc/trust-core test` | [`05-test-trust-core.txt`](./test-outputs/05-test-trust-core.txt) | `Tests: 33 passed, 33 total` |
| 6 | `npm run smoke:trust-api` | [`06-smoke-trust-api.txt`](./test-outputs/06-smoke-trust-api.txt) | `smoke OK` |
| 7 | `npm run smoke:trust-api:paid-mock` | [`07-smoke-trust-api-paid-mock.txt`](./test-outputs/07-smoke-trust-api-paid-mock.txt) | `paid-mock OK` (paid · settle-fail · deep-quote · deep-501) |
| 8 | `npm run test:mcp-server` | [`08-test-mcp-server.txt`](./test-outputs/08-test-mcp-server.txt) | `inspector OK` · `inspector-http OK` · `inspector-paid OK` |
