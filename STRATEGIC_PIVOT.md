...
**Status:** W11 — Public trust surface live. Four browser routes (`/trust/[target]`, `/passport/[address]`, `/agents`, `/docs`) render against the existing trust-api + Passport sdk. Eight legacy marketplace prefixes 308-redirect to `/legacy` via Next middleware; the `/legacy` explainer explains the freeze and links the eleven preserved contract names. Footer rewritten for the trust-layer surface. ESLint `no-restricted-imports` rule armed against `legacy-primitives/**` and waits for the codemod. Mass file move + indexer extraction explicitly deferred per the 90-day plan's "highest-blast-radius" framing; deferral plan in `docs/w11-followups.md`.
...
| W12 | Mass file move (`frontend` → `apps/web`, `backend` → `apps/indexer`, legacy contracts + marketplace pages → `legacy-primitives/`) + design-partner evidence object + load testing + security review |
