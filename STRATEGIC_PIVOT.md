...
**Status:** W13 — trust-api → real on-chain reads wired. `@arc/attestation-reader` ships as a read-only viem client for AttestationRegistry; `apps/trust-api/src/sources/passport.ts` assembles `{ passport, attestations[] }` envelopes by walking the W8-W12.4 canonical schema allowlist via Promise.all. `GET /v1/passport/:address` and the new `GET /v1/attestations/:subject` consume the deployed contracts when `ARC_PASSPORT_ADDRESS` + `ARC_ATTESTATION_REGISTRY_ADDRESS` + `ARC_RPC_URL` are set; they fall back to the W8 placeholder / 503-unconfigured otherwise (CI + unfunded deployments unchanged). W8-W12 consolidation milestone (`MILESTONE_W8_W12.md`) + four hardening items (body validators, adapter-swap runbook, registry-allowlist module + docs, package metadata sync) all landed prior. The W11 codemod (mass file move + apps/indexer extraction) remains the single deferred item, scoped in `docs/w11-followups.md`.
